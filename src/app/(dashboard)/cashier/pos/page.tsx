...
export default function POSPage() {
  ...
  // Fetch the open session when component mounts
  useEffect(() => {
    const fetchOpenSession = async () => {
      ...
      try {
        ...
        if (json.data && json.data.length > 0) {
          setSessionId(json.data[0].id);
        } else {
          // If no session is open, redirect to cashier dashboard
          router.push('/cashier');
        }
      } catch (error) {
  ...
  // Periodically check if session is still open (every 30 seconds)
  useEffect(() => {
    if (!sessionId) return;
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (sessionData.status === 'CLOSED') {
          // If session is closed, redirect to cashier dashboard
          router.push('/cashier');
        }
      } catch (error) {
  ...
  const processPayment = async () => {
    ...
    if (!sessionId) {
      setPaymentError("No active session found");
      return;
    }
    ...
    return (
      <div className="flex items-center justify-center h-[70vh]">
        <div className="text-center">
          ...
          <p className="mt-3 text-gray-500">Memeriksa sesi kasir...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex gap-4 h-[calc(100vh-7rem)]">
      ...
      <div className="w-96 flex-shrink-0 flex flex-col bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
          ...
          {cart.length > 0 && (
            <button onClick={() => setCart([])} className="text-xs text-red-500 hover:text-red-600 transition-colors">
              Hapus Semua
            </button>
          )}
        </div>
            ...
            cart.map((item) => (
              <div key={item.productId} className="px-4 py-2.5 flex items-center gap-2">
                ...
                <div className="w-20 text-right flex-shrink-0">
                  <p className="text-sm font-semibold text-gray-900">{formatIDR(item.price * item.qty)}</p>
                </div>
                <button
