      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          ...
          <p className="text-xs text-gray-400 mt-1">
            {data.today.customers} transaksi total
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          ...
          <p className="text-3xl font-bold text-gray-900">
            {formatIDR(data.month.revenue)}
          </p>
          <p className="text-xs text-gray-400 mt-1">
            {data.month.orders} pesanan bulan ini
          </p>
        </div>

        {/* Low Stock */}
        <div
          className={`bg-white rounded-xl shadow-sm border p-5 ${data.totals.lowStockProducts > 0 ? "border-red-200" : "border-gray-100"}`}
        >
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-medium text-gray-500">
              Stok Menipis
            ...
            <span
              className={`p-2 rounded-lg ${data.totals.lowStockProducts > 0 ? "bg-red-50 text-red-600" : "bg-gray-50 text-gray-400"}`}
            >
              <svg
                className="w-5 h-5"
                fill="none"
          ...
          <p
            className={`text-3xl font-bold ${data.totals.lowStockProducts > 0 ? "text-red-600" : "text-gray-900"}`}
          >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide py-3 pl-5 pr-4">
                    No. Pesanan
                  </th>
                  ...
                  <th className="text-center text-xs font-semibold text-gray-500 uppercase tracking-wide py-3 pr-4">
                    Status
                  </th>
                  <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide py-3 pr-5">
                    Waktu
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {data.recentOrders.length === 0 ? (
                  ...
                  data.recentOrders.map((order) => (
                    ...
                    >
                      <td className="py-4 pl-5 pr-4 font-mono text-blue-600 font-medium tracking-tight">
                        {order.orderNumber}
                      </td>
                      <td className="py-4 pr-4 text-gray-700 font-medium">
                        {order.cashier?.name ?? "-"}
                      </td>
                      <td className="py-4 pr-4 text-right font-semibold text-gray-900 tabular-nums">
                        {formatIDR(order.total)}
                      </td>
                      <td className="py-4 pr-4 text-center">
                        <span
                          className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${STATUS_CLASSES[order.status] ?? "bg-gray-100 text-gray-600"}`}
                        >
                          {STATUS_LABELS[order.status] ?? order.status}
                        </span>
                      </td>
                      <td className="py-4 pr-5 text-gray-500 text-sm tabular-nums">
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="text-right text-xs font-semibold text-gray-500 uppercase tracking-wide py tastable-nums py-3 pr-4">
                    Pendapatan
                  </th>
                  <th className="text-right text-xs font-semibold text-gray-500 uppercase tracking-wide py-3 pr-5">
                    Rata-rata
                  </th>
                </tr>
              </thead>
                ...
                {data.topProducts.length === 0 ? (
                  ...
                  data.topProducts.map((product) => (
                    <tr
                      ...
                      className="hover:bg-gray-50 transition-colors duration-150"
                    >
                      <td className="py-4 pl-5 pr-4 text-gray-800 font-medium">
                        {product.name}
                      </td>
                      <td className="py-4 pr-4 text-right text-gray-700 font-medium tabular-nums">
                        {product.totalSold}
                        <span className="text-gray-500 ml-1">pcs</span>
                      </td>
                      <td className="py-4 pr-4 text-right font-semibold text-gray-900 tabular-nums">
                        {formatIDR(product.revenue)}
                      </td>
                      <td className="py-4 pr-5 text-right text-gray-600 tabular-nums">
                        {formatIDR(
                          Math.round(
                            product.revenue / Math.max(1, product.totalSold),
                          ),
                        )}
                      </td>
                    </tr>
